...
input_shape = (seq_length, 3) #3개가 의미 하는 것은 pitch, step, duration
learning_rate = 0.01
...
x = tf.keras.layers.Dense(256)(x)
outputs = {
    'pitch': tf.keras.layers.Dense(128, name='pitch')(x), #무한의 숫자를 128개로 지정함
    'step': tf.keras.layers.Dense(1, name='step')(x), #하나의 값이 나오게 끔 함
    'duration': tf.keras.layers.Dense(1, name='duration')(x),#하나의 값이 나오게 끔 함
} #총 130개의 값이 나옴
